...
// Helpers
static void render_space(void) { oled_write_ln_P(PSTR(""), false); }
...
void render_modifiers(void) {
  ...
  (is_caps_word_on()) ? oled_write_P(PSTR("W"), false)
                      : oled_write_P(PSTR("*"), false);
  (modifiers & MOD_MASK_GUI) ? oled_write_P(PSTR("G"), false)
                             : oled_write_P(PSTR("*"), false);
  (modifiers & MOD_MASK_ALT) ? oled_write_P(PSTR("A"), false)
                             : oled_write_P(PSTR("*"), false);
  (modifiers & MOD_MASK_CTRL) ? oled_write_P(PSTR("C"), false)
                              : oled_write_P(PSTR("*"), false);
  (modifiers & MOD_MASK_SHIFT) ? oled_write_P(PSTR("S"), false)
                               : oled_write_P(PSTR("*"), false);
}
...
static void render_crkbd_logo(void) {
  static const char PROGMEM crkbd_logo[16] = {
      0x80, 0x81, 0x82, 0x83, 0x84, 0xa0, 0xa1, 0xa2,
      0xa3, 0xa4, 0xc0, 0xc1, 0xc2, 0xc3, 0xc4, 0};
  oled_write_P(crkbd_logo, false);
}

void render_kapi(void) {
  static const char PROGMEM kapi[2][16] = {
      {0x8a, 0x8b, 0x8c, 0x8d, 0x8e, 0xaa, 0xab, 0xac, 0xad, 0xae, 0xca, 0xcb,
       0xcc, 0xcd, 0xce, 0},
      {0x85, 0x86, 0x87, 0x88, 0x89, 0xa5, 0xa6, 0xa7, 0xa8, 0xa9, 0xc5, 0xc6,
       0xc7, 0xc8, 0xc9, 0},
  };
